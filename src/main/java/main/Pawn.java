...
public class Pawn
  ...
  public boolean move(ChessGameBoard board, int row, int col) {
    if (super.move(board, row, col)) {
      ...
      if ((getColorOfPiece() == ChessGamePiece.BLACK && row == 7)
          || (getColorOfPiece() == ChessGamePiece.WHITE && row == 0)) { // pawn has reached the end of the board,
                                                                        // promote it to queen
  ...
  protected ArrayList<String> calculatePossibleMoves(ChessGameBoard board) {
    ...
    if (isPieceOnScreen()) {
      ...
      checkForEnemies(board, moves);
    }
    return moves;
  }

  private void checkForEnemies(ChessGameBoard board, ArrayList<String> moves) {
  ...
  public ImageIcon createImageByPieceType() {
    if (getColorOfPiece() == ChessGamePiece.WHITE) {
      return new ImageIcon(
          getClass().getClassLoader().getResource("chessImages/WhitePawn.gif"));
    } else if (getColorOfPiece() == ChessGamePiece.BLACK) {
      return new ImageIcon(
          getClass().getClassLoader().getResource("chessImages/BlackPawn.gif"));
    } else {
      return new ImageIcon(
          getClass().getClassLoader().getResource("chessImages/default-Unassigned.gif"));
    }
  }
}
