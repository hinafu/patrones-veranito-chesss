  ...
  protected ArrayList<String> calculateSouthMoves(
    ...
    if (isPieceOnScreen()) {
      for (int i = pieceRow + 1; i < 8 && count < numMoves; i++) {
        boolean emptyCell = board.getCell(i, pieceColumn).getPieceOnSquare() == null;
        if (emptyCell || isEnemy(board, i, pieceColumn)) {
  ...
  protected ArrayList<String> calculateEastMoves(
    ...
    if (isPieceOnScreen()) {
      for (int i = pieceColumn + 1; i < 8 && count < numMoves; i++) {
        boolean emptyCell = board.getCell(i, pieceColumn).getPieceOnSquare() == null;
        if (emptyCell || isEnemy(board, pieceRow, i)) {
    ...
    if (isPieceOnScreen()) {
      for (int i = 1; i < 8 && count < numMoves; i++) {
        if (isOnScreen(pieceRow - i, pieceColumn - i)) {
          boolean emptyCell = board.getCell(pieceRow - i, pieceColumn - i).getPieceOnSquare() == null;
          if (getOr(emptyCell, isEnemy(board, pieceRow - i, pieceColumn - i))) {
